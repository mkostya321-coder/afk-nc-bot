...
# --------------------- Еженедельный отчёт по выплатам (четверг 8:00) ---------------------
    ...
    while True:
        ...
        try:
            ...
            if users:
                ...
                # Обнуляем баланс у всех, кто попал в отчёт
                if user_ids:
                    with sqlite3.connect(DB_PATH) as conn:
                        ...
                        conn.commit()
                    logging.info(f"✅ Обнулены балансы у {len(user_ids)} пользователей после отправки отчёта.")
            else:
                await bot.send_message(
                    chat_id=REPORT_CHAT_ID,
...
async def main():
    ...
    # dp.include_router(referral.router)   # закомментирован (рефералка уже в user.py)
